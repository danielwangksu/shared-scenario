...
void main(int argc, char **argv){
	...
	printf("alarmSensor: tempSen_pid=%d, tempCnt_pid=%d, heatAct_pid=%d, alarmAct_pid=%d,web_pid=%d, tempSen_ep=%d, tempCnt_ep=%d, heatAct_ep=%d, alarmAct_ep=%d, web_ep=%d\n", tempSen_pid, tempCnt_pid, heatAct_pid, alarmAct_pid, web_pid, tempSen_ep, tempCnt_ep, heatAct_ep, alarmAct_ep, web_ep);
	...
	while(1){
		...
		printf("alarmActuator: paststatus=%d, receiving message turning the alarm %s\n", paststatus, (m_in.m_type ? "ON" : "OFF"));
